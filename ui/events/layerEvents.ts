...
import { getLayerRect } from "../render/helper";
...
function getNextHitLayerIndex(event: MouseEvent, fallbackIndex: number): number {
    ...
    let hitIndices = state.current.layers
        ...
        .filter(item => item.layer
            ...
            && containsPoint(getLayerRect(item.layer), point))
    ...
    let topIndex = hitIndices[0];
    let topLayer = state.current.layers[topIndex];

    let signature = buildHitSignature(point, hitIndices);
    if (hitIndices.includes(fallbackIndex) && fallbackIndex !== state.selectedIndex) {
    ...
    if (signature !== lastHitSignature || state.selectedIndex === undefined) {
        lastHitSignature = signature;
        if (hitIndices.includes(fallbackIndex)) {
            let fallbackLayer = state.current.layers[fallbackIndex];
            if (shouldPreferTopHitOnInitialSelection(topLayer, fallbackLayer)) {
                return topIndex;
            }
            return fallbackIndex;
        }
...
function compareHitLayer(a: any, b: any, aIndex: number, bIndex: number): number {
    ...
    let areaDiff = getLayerArea(a) - getLayerArea(b);
    if (areaDiff !== 0) return areaDiff;
    return bIndex - aIndex;
}

function getLayerClickPriority(layer: any): number {
    if (layer.type === SMType.text) return 0;
    if (layer.type === SMType.slice) return 1;
    if (layer.type === SMType.shape) return 2;
    if (layer.type === SMType.symbol) return 3;
    if (layer.type === SMType.group) return layer.isStackLayout ? 4 : 5;
    if (layer.type === SMType.hotspot) return 99;
    return 6;
}

function getLayerArea(layer: any): number {
    let rect = getLayerRect(layer);
    if (!rect) return Number.MAX_SAFE_INTEGER;
    return Math.max(0, rect.width || 0) * Math.max(0, rect.height || 0);
}

function shouldPreferTopHitOnInitialSelection(topLayer: any, fallbackLayer: any): boolean {
    if (!topLayer || !fallbackLayer) return false;
    return getLayerClickPriority(topLayer) < getLayerClickPriority(fallbackLayer);
}
