...
export function renderCodeTemplate(layerData: LayerData): string {
    ...
    if (layerData.type == "text") {
        ios.push('<div id="ios-panel"  class="code-item item">', '<label><textarea id="css" rows="6" readonly="readonly">' +
            ...
            "label.frame = CGRectMake(" + scaleSize(rect.x) + "\, " + scaleSize(rect.y) + "\, " +
            scaleSize(rect.width) + "\, " + scaleSize(rect.height) + ");\r\n" +
        ...
        ios.push('<div id="ios-panel" class="code-item item">', '<label><textarea id="css" rows="6" readonly="readonly">' +
            ...
            "view.frame = CGRectMake(" + scaleSize(rect.x) + "\, " + scaleSize(rect.y) + "\, " +
            scaleSize(rect.width) + "\, " + scaleSize(rect.height) + ");\r\n" +
        ...
        ios.push('<div id="ios-panel" class="code-item item">', '<label><textarea id="css" rows="6" readonly="readonly">' +
            ...
            "imageView.frame = CGRectMake(" + scaleSize(rect.x) + "\, " + scaleSize(rect.y) + "\, " +
            scaleSize(rect.width) + "\, " + scaleSize(rect.height) + ");\r\n" +
...
function getAndroidWithHeight(layerData: LayerData) {
    let rect = getLayerRect(layerData);
    return "android:layout_width=\"" + unitSize(rect.width, false) + "\"\r\n" + "android:layout_height=\"" + unitSize(rect.height, false) + "\"\r\n";
}
