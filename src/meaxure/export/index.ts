...
export async function exportSpecification() {
    const RUNNING_FLAG_KEY = "com.istraw.sketch-hmeaxure.exporting"
    ...
    try {
        ...
        safeCall(() => writeExportReport(results.advancedMode, data.artboards.length));
...
function writeExportReport(advancedMode: boolean, artboardCount: number) {
    writeFile({
        content: JSON.stringify({
            exportedAt: new Date().toISOString(),
            durationMs: stopwatch.elpased(),
        ...
        path: savePath,
        fileName: "meaxure-export-report.json"
    });
}
