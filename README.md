# Sketch MeaXure Plus

[English](#english) | [中文](#中文)

---

<a name="english"></a>
## English
### 🚀 Key Updates
* Full Compatibility: Now fully supports Sketch 2026.1.1.
* Cascading Styles: Added support for displaying Cascading Styles (CSS) to better handoff design details.
* Slice Area Visualization: Support for displaying Slice Areas to ensure export accuracy.
* Mixed-Color Text Support: Support for selecting and viewing color values of individual segments within mixed-color text boxes.
* Bug Fixes: 
  - Fixed issues related to Symbol instance exports.

### 📦 Installation
1. Go to the Releases page.
2. Download the latest .sketchplugin.zip.
3. Unzip and double-click the file to install.

### 📝 Acknowledgment
This project is inspired by the pioneering work of utom and we would like to express our sincere gratitude to qjebbs for their long-term maintenance of the original repository.

### 🛠 Maintenance Statement
Since the original project is no longer actively maintained, I have taken over the responsibility for its ongoing development. My goal is to keep this essential tool alive and fully functional for modern design workflows.


---

<a name="中文"></a>
## 中文
### 🚀 核心更新
* 版本兼容：现已全面支持 Sketch 2026.1.1 及更高版本。
* 层叠样式显示：新增支持显示 层叠样式 (CSS)，方便更精准地还原设计细节。
* 切图范围展示：支持展示 切图范围，确保导出的切图资源位置和尺寸精准无误。
* 多色文本支持：支持选中并查看同一文本框中不同颜色文字段落的独立色值。
* 缺陷修复：
  - 修复了 Symbol（符号） 实例在导出过程中的失效及报错问题。

### 📦 安装方式
1. 前往 Releases 发布页面。
2. 下载最新的 .sketchplugin.zip 压缩包。
3. 解压并双击 .sketchplugin 文件即可完成安装。

### 📝 致谢与传承
本项目深受 utom 开创性工作的影响，并在此特别感谢 qjebbs 对原仓库的长期维护与贡献。

### 🛠 维护声明
鉴于原版已停止维护，本项目目前由我接手承担后续的开发与维护责任。我致力于确保这款经典的插件在最新的设计环境中依然稳定、高效。
